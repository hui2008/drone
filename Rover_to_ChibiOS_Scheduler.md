...
classDiagram
    ...
    class Rover {
      file: ardupilot/Rover/Rover.cpp
      path: ardupilot/Rover/Rover.h
      global rover
      get_scheduler_tasks()
      scheduler_tasks[]
    }
    ...
    class AP_Vehicle {
      file: ardupilot/libraries/AP_Vehicle/AP_Vehicle.cpp
      path: ardupilot/libraries/AP_Vehicle/AP_Vehicle.h
      setup()
      loop() final
      AP_Scheduler scheduler
    }
    ...
    class HAL_Callbacks {
      file: ardupilot/libraries/AP_HAL/HAL.h
      interface setup()
      interface loop()
    }

    class AP_HAL_MAIN_CALLBACKS {
      file: ardupilot/libraries/AP_HAL/AP_HAL_Main.h
      generates main()
      calls hal.run(..., callbacks)
    }

    class HAL_ChibiOS {
      file: ardupilot/libraries/AP_HAL_ChibiOS/HAL_ChibiOS_Class.cpp
      path: ardupilot/libraries/AP_HAL_ChibiOS/HAL_ChibiOS_Class.h
      run(argc, argv, callbacks)
      main_loop()
      owns schedulerInstance
    }

    class AP_HAL_HAL {
      file: ardupilot/libraries/AP_HAL/HAL.h
      stores hal.scheduler
      virtual run(...)
    }

    class ChibiOS_Scheduler {
      file: ardupilot/libraries/AP_HAL_ChibiOS/Scheduler.cpp
      path: ardupilot/libraries/AP_HAL_ChibiOS/Scheduler.h
      init()
    ...
    class AP_HAL_Scheduler {
      file: ardupilot/libraries/AP_HAL/Scheduler.h
      platform scheduler interface
    }

    class AP_Scheduler {
      file: ardupilot/libraries/AP_Scheduler/AP_Scheduler.cpp
      path: ardupilot/libraries/AP_Scheduler/AP_Scheduler.h
      init(tasks, count, log_bit)
      loop()
      run(time_available)
    }

    Rover --|> AP_Vehicle : inherits
    AP_Vehicle ..|> HAL_Callbacks : implements setup/loop
    AP_HAL_MAIN_CALLBACKS --> HAL_ChibiOS : calls hal.run(&rover)
    HAL_ChibiOS --|> AP_HAL_HAL : inherits
    HAL_ChibiOS *-- ChibiOS_Scheduler : owns schedulerInstance
    ChibiOS_Scheduler --|> AP_HAL_Scheduler : implements platform scheduler API
    AP_HAL_HAL o-- AP_HAL_Scheduler : hal.scheduler
    HAL_ChibiOS --> HAL_Callbacks : stores as g_callbacks
    HAL_ChibiOS --> AP_Vehicle : g_callbacks->setup()/loop()
    AP_Vehicle *-- AP_Scheduler : owns vehicle scheduler
    AP_Vehicle --> AP_Scheduler : loop() calls scheduler.loop()
    Rover --> AP_Scheduler : supplies scheduler_tasks[]
```
